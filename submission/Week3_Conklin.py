...
for i, df in enumerate(dfs):
  df['id'] = "sub"+str(i+1)    # if i is 0, this should be repeated accounts of 'sub1'
  df['time'] = pd.Categorical( ['pre', 'test', 'post'], categories=['pre','test','post'], ordered=True)
  df = df.iloc[:, [2, 3, 0, 1]] #reorders collumns
  dfs[i] = df #puts changes to each df back in dfs list
...
# %% Complete the code
back_to_long.sort_values(by=['id','time','variable']).reset_index(drop=True).iloc[:, [0, 3, 1, 2]]
...
#           4.4 |           5.5 |
# ------------------------------+
wide_form.loc[:,['Measure_A-pre','Measure_B-pre']]
...
# --------------+----------------+----------------|
#           3.2 |            7.8 |            5.1 |
wide_form.loc[0,['Measure_A-pre','Measure_A-test','Measure_A-post']]
...
long_form.loc[
    (long_form['time'] == 'pre') &
    (long_form['variable'].str.endswith('B')),
    :
]
